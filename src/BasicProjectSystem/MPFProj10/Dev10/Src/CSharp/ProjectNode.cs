		...
		IVsSetTargetFrameworkWorkerCallback
    {
        ...
        public ImageHandler ImageHandler
        {
            get
            {
                ...
                {
                    imageHandler = new ImageHandler(typeof(ProjectNode).Assembly.GetManifestResourceStream("SimpleProjectPackage.Resources.imagelis.bmp"));
                }
                return imageHandler;
            }
        }
...
#warning workarroud: rturns allways true
                    result = this.InvokeMSBuild2(target);
                    //result = this.InvokeMSBuild(MSBuildTarget.Clean);
                    //result = MSBuildResult.Successful;
                }
                finally
                {
        ...
        protected virtual MSBuildResult InvokeMSBuild2(string target)
        {
            MSBuildResult result = MSBuildResult.Failed;
            const bool designTime = true;
            bool requiresUIThread = UIThread.Instance.IsUIThread; // we don't run tasks that require calling the STA thread, so unless we're ON it, we don't need it.
            ...
            if (this.buildProject != null)
            {
                ...
                try
                {
                    ...
                    //BuildRequestData requestData = new BuildRequestData(currentConfig, targetsToBuild, this.BuildProject.ProjectCollection.HostServices, BuildRequestDataFlags.ReplaceExistingProjectInstance); // the flag "BuildRequestDataFlags.ReplaceExistingProjectInstance" causes the ctor to fail.
                    //BuildRequestData requestData = new BuildRequestData(this.CurrentConfig.FullPath, new Dictionary<string, string>(), null, targetsToBuild, null); //this works
                    BuildRequestData requestData = new BuildRequestData(this.CurrentConfig.FullPath, new Dictionary<string, string>(), null, targetsToBuild, this.BuildProject.ProjectCollection.HostServices);// this works
                    submission = BuildManager.DefaultBuildManager.PendBuildRequest(requestData);
                    if (accessor != null)
                    {
                        ErrorHandler.ThrowOnFailure(accessor.RegisterLogger(submission.SubmissionId, this.buildLogger));
                    ...
                    //#error This line generates an error.
                    //The buildSubmission class throws an error tha the Base class is not in the assemblie, but it muss be there because both classes are in the same assembly.
                    // BuildResult buildResult = submission.Execute();
                    BuildResult buildResult = BuildManager.DefaultBuildManager.BuildRequest(requestData);
